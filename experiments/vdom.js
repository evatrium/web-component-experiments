import {isFunc, isObj, isArray, removeListener as remLstnr} from "@iosio/util";
...
export const removeHandlers = (dom) => {
    [].concat(...dom.childNodes).forEach(c => {
        if (c.handlers) for (let k in c.handlers) remLstnr(c, k, listener);
        removeHandlers(c);
    });
};
...
var createNode = function (vnode, isSvg) {
    var node =
        vnode.type === TEXT_NODE
            ...
            /*
              START - ALTERATIONS
              implements the use of document fragments
...
var patchNode = (parent, node, oldVNode, newVNode, isSvg) => {
    ...
    if (oldVNode === newVNode) {
        ...
        /*
            ...
            as opposed to just comparing string values.
        ...
        for (var i in merge(oldVProps, newVProps)) {
            ...
            if ((i === "value" || i === "selected" || i === "checked" ? node[i] : _old) !== _new) {
                // console.log('changed', _old, _new);
                patchProperty(node, i, _old, _new, isSvg)
            }
            // i === 'className' && console.log(oldVProps, newVProps)
